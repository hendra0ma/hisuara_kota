...
$config->regencies_id =  (string) $regency_id->regency_id;
    ...
    <div class="row" style="margin-top: 90px; transition: all 0.5s ease-in-out;">
        ...
        <div class="col-lg-12">
            ...
            <ul class="breadcrumb">
                <?php $regency = Regency::where('id', $config->regencies_id)
                    ->select('name')
                    ->first(); ?>
            ...
            <div class="col-lg" style="{{ $config->quick_count == 'yes' ? '' : 'display:none' }}">
                <div class="card" style="margin-bottom: 1rem">
                    <div class="card-header"
                        style="background: linear-gradient(90deg, rgba(11,53,217,1) 0%, rgba(23,154,236,1) 100%);">
...
<div style="
    ...
    <script>
        ...
        `;
            ...
            var ajaxRequests = [{
                    url: "{{ url('') }}/administrator/get_realcount_ajax",
                    container: "#container-realcount"
                },
                {
                    url: "{{ url('') }}/administrator/get_terverifikasi_ajax",
                    container: "#container-terverifikasi"
                },
                {
                    url: "{{ url('') }}/administrator/get_quickcount_ajax",
                    container: "#container-quickcount"
                },
                {
                    url: "{{ url('') }}/administrator/get_rekapitulasi_ajax",
                    container: "#container-rekapitulasi"
                },
                {
                    url: "{{ url('') }}/administrator/get_kpu_ajax",
                    container: "#container-kpu"
                },
                ...
                if (index < Object.keys(data).length) {
                    const {
                        ...
                        nomor_tps
                            ...
                            $('#container-marquee').append(`
                                <span class="text-white fw-bold" style="margin-top:8px;display:none;flex:0 0 auto; width:auto;margin-left:10px;font-size:15px">${username}, ${nama_kelurahan},TPS ${nomor_tps}</span>
                            `);
                }
            ...
            setTimeout(() => {
                let id_wilayah =
                    {{ request()->segment(3) != null ? Crypt::decrypt(request()->segment(3)) : $config->regencies_id }};
                let tables = ['saksi', 'crowd_c1'];
                $.ajax({
                    url: "{{ route('getMarquee') }}",
                    data: {
                        tables,
                        id_wilayah,
                        limit: 10
