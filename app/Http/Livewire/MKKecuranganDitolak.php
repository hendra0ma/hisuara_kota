...
class MKKecuranganDitolak extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $data['jumlah_kecurangan_ditolak']  = Tps::join('saksi', 'saksi.tps_id', '=', 'tps.id')
        ...
        $data['tidak_menjawab'] = Saksi::where('kecurangan', 'yes')->where('status_kecurangan', 'terverifikasi') ->where("saksi.regency_id",  $this->config->regencies_id)->where('makamah_konsitusi', 'Tidak Menjawab')->get();
        $data['selesai'] = Saksi::where('kecurangan', 'yes')->where('status_kecurangan', 'terverifikasi') ->where("saksi.regency_id",  $this->config->regencies_id)->where('makamah_konsitusi', 'Selesai')->get();
        ...
        return view('livewire.m-k-kecurangan-ditolak', $data);
    }
}
